...
class CloseRequests(BasePage):
    ...
    @staticmethod
    def get_index_for_change_number(
        change_number: str, list_of_change_number: list
    ) -> Union[int, None]:
    ...
    def find_the_change_request(self, change_number: str, index: int) -> None:
        ...
        final_xpath = (
            "//table[@id='T301444200']//tr[" + str(index) + "]//td[1]/nobr[1]/span"
        )
        ...
        try:
            ...
            self.__set_change_number(
                change_number
            )  # set the change number for future use
    ...
    def __common_closing_activity(self, start_time: str) -> None:
        ...
        self.write(
            Close_Locators.TASK_ACTUAL_START_DATE,
            _open_time.strftime("%m/%d/%Y %I:%M:%S %p"),
        )
        if self.get_change_type():
            ...
            _actual_end_date: str = make_downtime_from_open_time_2(
                _open_time.strftime("%m/%d/%Y %I:%M:%S %p"), _start_date, _end_date
            )
            self.write(Close_Locators.TASK_ACTUAL_END_DATE, _actual_end_date)
        else:
            self.write(
                Close_Locators.TASK_ACTUAL_END_DATE,
                _open_time.strftime("%m/%d/%Y %I:%M:%S %p"),
            )
    ...
    def close_service_downtime_window_task(
        self, actual_start_time: str, current_time_of_user: str
    ) -> None:
        """
        ...
        try:
            ...
            element = WebDriverWait(self._driver, self.timeout).until(
                ec.visibility_of_element_located(
                    Task_Locators.SERVICE_DOWNTIME_WINDOW_TASK_SPAN
                )
            )
    ...
    def goto_next_stage(self) -> None:
        ...
        if not self.is_change_status_closed():
            ...
            self.handle_frame_alert(
                Frame_Locators.FRAME_OF_CONFIRMATION, Frame_Locators.FRAME_OK_BUTTON
            )
        else:

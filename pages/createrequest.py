...
class CreateRequests(BasePage):
        ...
        while self.__change_number == "":
            try:
                ...
                raise NoSuchElementException(
                    "SET_CHANGE_NUMBER | NO SUCH ELEMENT EXCEPTION"
                )
    ...
    def select_change_manager(self, change_manager: str) -> None:
        ...
        if change_manager == self.TNR_GROUP[0]:
            ...
            print(
                f"{Colors.WARNING}Manager Not Found ! Using Default Manager: {self.TNR_GROUP[0]}{Colors.ENDC}"
            )
            self.click(ChangeManagerLocators.CHANGE_MANAGER_SHAHED)
        ...
        for child_window in self._driver.window_handles:
            if child_window != parent_window:
                ...
                for grand_child_window in self._driver.window_handles:
                    if (
                        grand_child_window != parent_window
                        and grand_child_window != child_window
                    ):
                        # Switch to the new Child window
                        self._driver.switch_to.window(grand_child_window)
                        # Insert all the necessary info from here
                        self.write(
                            LocationServiceLocators.SITE_TEXTBOX,
                            change_location_details[1],
                        )
    ...
    def fill_service_downtime_duration_task(
    ...
    def fill_review_closure_task(
        self, close_start_time: str, close_end_time: str
    ) -> None:
    ...
    def go_back_to_homepage(self) -> None:
        ...
        try:
            self.wait_for_loading_icon_disappear(
                *CommonChangeCreateLocators.LOADING_ICON
            )
    ...
    def __set_date_time_in_task(
        self, parent_window: object, start_time: str, end_time: str
    ) -> None:
    ...
    def verify_summary(self, summary: str):
        ...
        contents = (
            self._driver.find_element(*SummaryAndNotesBox.SUMMARY_TEXTBOX)
            .get_attribute("value")
            .strip()
        )

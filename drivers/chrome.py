...
class ChromeDriver:
    ...
    def setUpDriver(cls):
        ...
        options.add_argument(
            "--log-level=3"
        )  # disable Info/Error/Warning in Chrome Driver
        options.add_experimental_option(
            "excludeSwitches", ["enable-logging"]
        )  # disable Dev Info info while running app
        options.add_argument(
            "--start-maximized"
        )  # start the chrome with maximized window
        os.environ[
            "WDM_LOG_LEVEL"
        ] = "0"  # Disable the logging of ChromeDriverManager()
        cls.browser: WebDriver = webdriver.Chrome(

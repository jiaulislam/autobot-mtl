...
class Writer:
    ...
    def write_to_excel(self, _db_data: List[Tuple]) -> None:
        self.___db_data_length += self.get_data_length(_db_data)
        ...
        for _row in range(1,len(_db_data)+1):
            for _idx, _col in enumerate(_db_data[_row-1]):
                if _idx == 0:
                    ...
                    # * no need to give user to create_at value
                    pass
                else:
    ...
    def __make_table(self):
        _options = {
            ...
            'columns': [{'header': i.upper()} for i in SheetHeader.get_header()]
        }
        self.worksheet.add_table(f'A1:L{self.___db_data_length}', options=_options)
